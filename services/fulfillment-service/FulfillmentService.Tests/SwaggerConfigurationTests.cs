...
public class SwaggerConfigurationTests : IClassFixture<FulfillmentServiceApplicationFactory>
{
    private readonly FulfillmentServiceApplicationFactory _factory;

    public SwaggerConfigurationTests(FulfillmentServiceApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public void SwaggerDefinesOAuth2FlowForKeycloak()
    {
        ...
        oauthScheme.Flows.AuthorizationCode.Scopes.Keys.Should().Contain(new[] { "openid", "profile" });
    }
    ...
    public async Task SwaggerUiEndpoint_IsReachableInDevelopment()
    {
        ...
        html.Should().Contain("Fulfillment Service API v1");
    }
}
